...
class ResultsScreen:
    ...
    def draw(self, screen, score, max_score):
        ...
        # Draw results box
        self._draw_results_box(screen, rank_info, score, max_score, percentage, display_percentage)
    ...
    def _draw_decorative_circles(self, screen, rank_info):
        ...
        for i in range(6):
            ...
            pygame.draw.circle(circle_surface, (*rank_info["color"], alpha), (size, size), size, 4)
    ...
    def _draw_rank_badge(self, screen, rank_info):
        ...
        pygame.draw.circle(rank_bg_surface, (*rank_info["color"], 150), (rank_size, rank_size), rank_size)
        pygame.draw.circle(rank_bg_surface, (*self.colors.WHITE, 200), (rank_size, rank_size), rank_size, 8)
    ...
    def _draw_stars(self, screen, rank_info):
        ...
        for i in range(rank_info["stars"]):
            ...
            self.renderer.draw_star(star_x, star_y + star_bounce, 20, rank_info["particle_color"])
    
    def _draw_results_box(self, screen, rank_info, score, max_score, percentage, display_percentage):
        ...
        for i in range(5, 0, -1):
            ...
            pygame.draw.rect(glow_surface, (*rank_info["color"], glow_alpha), glow_surface.get_rect(), border_radius=25)
        ...
        pygame.draw.rect(box_surface, rank_info["color"], box_surface.get_rect(), width=4, border_radius=25)
        ...
        # Percentage bar
        self._draw_percentage_bar(screen, box_y, rank_info, percentage, display_percentage)
    
    def _draw_percentage_bar(self, screen, box_y, rank_info, percentage, display_percentage):
        ...
        # Bar background
        pygame.draw.rect(screen, (40, 40, 60), (bar_x, bar_y, bar_width, bar_height), border_radius=15)
        ...
        # Bar border
        pygame.draw.rect(screen, self.colors.WHITE, (bar_x, bar_y, bar_width, bar_height), 
                        width=3, border_radius=15)
        
        # Percentage text
        percent_text = f"{int(percentage)}%"
        percent_surface = self.fonts.get_medium_font().render(percent_text, True, self.colors.WHITE)
    ...
    def _draw_confetti(self, screen):
        ...
        for i in range(15):
            ...
            size = 8 + (i % 3) * 4
            colors = [self.colors.YELLOW, self.colors.PINK, self.colors.CYAN, 

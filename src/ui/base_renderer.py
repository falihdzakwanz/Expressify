...
class UIRenderer:
    ...
    def draw_text_with_glow(self, text, font, color, x, y):
        ...
        for i in range(5, 0, -1):
            ...
            alpha_surface = pygame.Surface(
                (glow_rect.width + i * 4, glow_rect.height + i * 4), pygame.SRCALPHA
            )
    ...
    def draw_fancy_button(self, text, x, y, color, pulse_size, is_hovered=False):
        """Draw a fancy button with animation and hover detection"""
        # Background glow - stronger when hovered
        glow_layers = 4 if is_hovered else 3
        for i in range(glow_layers):
            alpha_value = (50 if is_hovered else 40) - i * 10
            ...
            self.screen.blit(glow_surface, (x - (200 + i * 10), y - (25 + i * 5)))

        # Button background - more opaque when hovered
        button_surface = pygame.Surface((400, 50), pygame.SRCALPHA)
        alpha = 150 if is_hovered else 100
        pygame.draw.rect(
            button_surface, (*color, alpha), button_surface.get_rect(), border_radius=25
        )
        ...
        # Text - always use symbol font for consistent unicode support
        text_surface = self.fonts.get_symbol_font().render(
        ...
        return pygame.Rect(x - 200, y - 25, 400, 50)

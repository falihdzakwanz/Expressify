...
class MenuScreen:
    ...
    def draw(self, screen, selected_index=0):
        ...
        for i in range(5):
            ...
            pygame.draw.circle(
                circle_surface, (*self.colors.PURPLE, alpha), (size, size), size, 3
            )
        ...
        self.renderer.draw_star(
            self.width - 200, subtitle_y - emoji_bounce, 25, self.colors.PINK
        )
    ...
    def _draw_rainbow_title(self, screen, title_text, title_y):
        ...
        for i, char_surface in enumerate(char_surfaces):
            ...
            shadow = self.fonts.get_title_font().render(
                title_text[i], True, (20, 20, 40)
            )
    ...
    def _draw_instructions_box(self, screen):
        ...
        for i in range(5, 0, -1):
            ...
            border_surface = pygame.Surface(
                (box_rect.width + i * 4, box_rect.height + i * 4), pygame.SRCALPHA
            )
            pygame.draw.rect(
                border_surface,
        ...
        pygame.draw.rect(
            box_surface, (30, 20, 50, 180), box_surface.get_rect(), border_radius=20
        )
        ...
        for text in instructions:
            # Decorative bullet
            pygame.draw.circle(
                screen, self.colors.YELLOW, (box_rect.x + 50, y_offset + 10), 8
            )
            pygame.draw.circle(
                screen, self.colors.WHITE, (box_rect.x + 50, y_offset + 10), 4
            )

            text_surface = self.fonts.get_small_font().render(
                text, True, self.colors.WHITE
            )
    ...
    def get_button_rects(self):
        ...
        for i in range(3):
            ...
            rects.append(pygame.Rect(box_x, menu_y, box_width, box_height))
        return rects
        ...
        for i, option in enumerate(menu_options):
            ...
            if is_selected or is_hovered:
                ...
                glow_rect = pygame.Rect(
                    ...
                    menu_y - 6 - pulse,
                    box_width + 12,
                    box_height + 12 + pulse * 2,
                )
                pygame.draw.rect(
                    screen, option["color"], glow_rect, border_radius=15, width=4
                )
            ...
            alpha = 240 if (is_selected or is_hovered) else 180
            pygame.draw.rect(
                box_surface,
                (30, 30, 30, alpha),
            ...
            # Text
            text_color = (
                option["color"] if (is_selected or is_hovered) else self.colors.GRAY
            )
            text_font = (
                self.fonts.get_medium_font()
                if (is_selected or is_hovered)
                else self.fonts.get_small_font()
            )
            text_surface = text_font.render(option["text"], True, text_color)
            text_rect = text_surface.get_rect(
                center=(box_x + box_width // 2, menu_y + box_height // 2)
            )
    ...
    def _draw_navigation_instructions(self, screen):
        ...
        inst_y = 520
        text_nav = " untuk navigasi • ENTER untuk pilih"
    ...
    def _draw_team_info(self, screen):
        ...
        team_title = self.fonts.get_small_font().render(
            "Tim Pengembang:", True, self.colors.YELLOW
        )
